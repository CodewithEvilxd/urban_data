...
_zones_cache: dict[str, Any] | None = None
...
def zone_row_dict(z) -> dict:
    return {
        "zone_id": str(getattr(z, "zone_id")),
        "mean_lst": float(getattr(z, "mean_lst")),
        "ndvi": float(getattr(z, "ndvi")),
        "ndbi": float(getattr(z, "ndbi")),
        "builtup_density": float(getattr(z, "builtup_density")),
        "impervious_fraction": float(getattr(z, "impervious_fraction")),
        "water_dist_m": float(getattr(z, "water_dist_m")),
        "latitude": float(getattr(z, "latitude")),
        "longitude": float(getattr(z, "longitude")),
        "heat_class": str(getattr(z, "heat_class")),
    }
...
def list_zones(city: str | None = None, bbox: str | None = None, limit: int = 5000):
    if use_db():
        ...
        for z in rows:
            ...
            props["recommendation_summary"] = str(getattr(z, "recommendation_summary", ""))
            city_slug = str(getattr(z, "city", ""))
            ...
            features.append(
                {
                    ...
                    "geometry": geometry_geojson(getattr(z, "geom")),
                    "properties": props,
                }
            )
        session.close()
...
def zone_detail(zone_id: str):
    if use_db():
        ...
        detail = add_place_context(zone_to_feature(z), str(getattr(z, "city", "")))
        detail["geometry"] = geometry_geojson(getattr(z, "geom"))
        detail["interventions"] = json.loads(str(getattr(z, "interventions_json", "[]")))
        detail.update(
...
def simulate(body: SimulateRequest):
    ...
    if use_db():
        ...
        current_class = str(getattr(z, "heat_class"))
        mean_lst = float(getattr(z, "mean_lst"))
        ndvi = float(getattr(z, "ndvi")) + body.ndvi_increase
    else:
...
def city_stats(city: str = "delhi", bbox: str | None = None):
    if bbox:
        ...
        if use_db() and rows:
            ...
            session.close()
            if first:
                scene_id = str(getattr(first, "scene_id"))
                scene_date = getattr(first, "scene_date").date().isoformat()
        elif rows:
            try:
                ...
                pass
        return compute_stats_dict(rows, city, str(scene_id), scene_date)
    ...
    if use_db():
        ...
        trend = [
            {
                "date": getattr(s, "scene_date").date().isoformat(),
                "mean_lst": round(float(getattr(s, "mean_lst")), 2),
            }
        ...
        base = {
            "city": city,
            "mean_lst": round(float(getattr(latest, "mean_lst")), 2),
            "pct_low": round(float(getattr(latest, "pct_low")), 1),
            "pct_moderate": round(float(getattr(latest, "pct_moderate")), 1),
            "pct_high": round(float(getattr(latest, "pct_high")), 1),
            "pct_critical": round(float(getattr(latest, "pct_critical")), 1),
            "critical_count": int(getattr(latest, "critical_count")),
            "scene_id": str(getattr(latest, "scene_id")),
            "scene_date": getattr(latest, "scene_date").date().isoformat(),
...
def automation_allowed(request: Request, secret: str | None) -> None:
    expected = os.getenv("AUTOMATION_SECRET")
    if not expected:
        raise HTTPException(503, "AUTOMATION_SECRET is not configured")
    provided = secret or request.headers.get("x-automation-secret")
    if not provided or not hmac.compare_digest(provided, expected):
        raise HTTPException(403, "Invalid automation secret")


def run_daily_update_job(retrain_model: bool, push: bool) -> None:
    with _automation_lock:
        _automation_status.update(
            {
                "running": True,
    ...
    try:
        result = subprocess.run(
            ...
            timeout=int(os.getenv("AUTOMATION_TIMEOUT_SECONDS", "7200")),
        )
        output_tail = (result.stdout or "")[-6000:]
...
def automation_status(request: Request, secret: str | None = None):
    automation_allowed(request, secret)
    return dict(_automation_status)


@app.get("/api/automation/daily-update")
def trigger_daily_update(
    ...
    with _automation_lock:
        if _automation_status["running"]:
            return {"status": "already_running", **_automation_status}
        _automation_status["running"] = True

    thread = threading.Thread(
        target=run_daily_update_job,
        args=(retrain_model, push),
        daemon=True,
    )
    thread.start()
    return {"status": "started", "retrain_model": retrain_model, "push": push}
